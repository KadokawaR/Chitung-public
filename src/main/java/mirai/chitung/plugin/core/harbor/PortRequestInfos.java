...
public class PortRequestInfos {
    // PERSONAL 用于限制响应来自特定用户的消息的条数，无论该消息是通过私聊还是群消息发出。阈值以 Bot 回复消息数量为准。
    // 意味着该 Bot 回应某用户的阈值为 10 条消息，每天阈值为 100 条消息，
    public final static PortRequestInfo PERSONAL = new PortRequestInfo("personal", 5, 100);
    final static int NOT_LIMITED = 99999;
    // GROUP_MINUTE 用于限制每分钟响应来自群的消息的条数。阈值以 Bot 回复消息数量为准。
    // 意味着该 Bot 回应某群每分钟阈值为 10 条消息。
    public final static PortRequestInfo GROUP_MINUTE = new PortRequestInfo("group", 10, NOT_LIMITED);
    // TOTAL_DAILY 用于限制每天响应所有来源的消息的条数。阈值以 Bot 回复消息数量为准。
    // 意味着 Bot 可以每天发出 4000 条消息。
