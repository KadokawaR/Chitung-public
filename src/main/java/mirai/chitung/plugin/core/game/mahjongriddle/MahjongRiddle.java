...
public class MahjongRiddle {
    ...
    //判定消息里面是否有答案
    public static boolean gotAnswer(int[] answerNum, GroupMessageEvent event,String message) {
    ...
    public static RiddleFactor setIsGuessed(RiddleFactor rf, GroupMessageEvent event,String message) {
    ...
    //临时性地检测是否是麻将牌用语
    public static boolean isMahjongTile(GroupMessageEvent event,String str) {
    ...
    public static void riddleStart(GroupMessageEvent event,String message){
        ...
        if (isMahjongTile(event,message) && riddleSessionHolder.containsKey(event.getGroup().getId())) {
            riddleSessionHolder.get(event.getGroup().getId()).isGuessed = setIsGuessed(riddleSessionHolder.get(event.getGroup().getId()), event,message).isGuessed;
            //
            if (gotAnswer(riddleSessionHolder.get(event.getGroup().getId()).answerNum, event,message)) {
